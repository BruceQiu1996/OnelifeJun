...
using LiJunSpace.ServerMode.Components.Shared;
...
namespace LiJunSpace.ViewModels
{
    public class EditRecordComponentViewModel : ComponentBase
    {
        ...
        protected async override Task OnInitializedAsync()
        {
            ...
            if (resp != null)
            {
                var record = JsonSerializer.Deserialize<RecordDto>(await resp.Content.ReadAsStringAsync(), MyHttpRequest._jsonSerializerOptions);
                if (record != null)
                {
                    Title = record.Title;
